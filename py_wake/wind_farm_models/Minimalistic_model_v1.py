...
Version: #1 - constant correction factor a = 3
...
def power_production_a3(Pg, CT, D, H, z0, Aw, kw, Nturb, Area):
    ...
    Description: Calculation of the annual energy production of the model developped
        by Jens N. Sørensen and Gunner C. Larsen
    ...
    eta0 = alpha * Aw**3 * gamma(1 + 3 / kw) * (gammainc(1 + 3 / kw, (Ur / Aw)**kw) - gammainc(1 + 3 / kw, (Uin / Aw)**kw)) + beta * (
        np.exp(-(Uin / Aw)**kw) - np.exp(-(Ur / Aw)**kw)) + np.exp(-(Ur / Aw)**kw) - np.exp(-(Uout / Aw)**kw)
...
class MinimalisticWindFarmModel_a3(WindFarmModel):
    ...
    def calc_wt_interaction(self, x_ilk, y_ilk, h_i=None, type_i=0,
        ...
        for i in range(n_sector):
            ...
            if sector_frequency[i] > 0:
                power_sector, ws_eff_sector = power_production_a3(Pg=rated_power,
                                                  CT=ct_max,
